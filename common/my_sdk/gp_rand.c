...
******************************************************************************/
/**** Static variables ****/
static int _r250_index = 0;
static unsigned int _r250_buffer[250] = {
...
static unsigned _achi_internal_rand();
static void _achi_internal_srand(unsigned newseed);
...
/*---------------------------------------------------------------------------*/
    if (_r250_index >= 147)
        j = _r250_index - 147;      /* Wrap pointer around */
    else
        j = _r250_index + 103;

    new_rand = _r250_buffer[_r250_index] ^= _r250_buffer[j];

    if (_r250_index >= 249)      /* Increment pointer for next time */
        _r250_index = 0;
    else
        _r250_index++;
...
static unsigned long _achi_internal_seed=1;
...
static unsigned _achi_internal_rand()
{
	_achi_internal_seed = _achi_internal_seed*0x015a4e35L + 1;
	return (_achi_internal_seed>>16)&0x7fff;
}

	/*	Initialize above linear congruent pseudorandom number generator */
static void _achi_internal_srand(unsigned newseed)
{	
	_achi_internal_seed = newseed;
}
