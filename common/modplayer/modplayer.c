...
volatile int st_snd = 0;
int use_timer,timer_num;
...
//playing module data	
	...
	int mod_speed;
	int mod_next_change;
static volatile int mod_playing=false;
...
// force restart
	if (mod_pattern_n>=127)  //loop the patterns
    		mod_pattern_n=0;
...
__inline void mod_player(void) {
    ...
    for (i=0;i<mod_channels_num;i++) {	// effects before pattern processing ---------------
    	
    		if (mod_channels[i]->portamento) {
    			mod_channels[i]->period+=mod_channels[i]->portamento;
    			mod_channels[i]->increment=freqtable[mod_channels[i]->period];
    		}
...
//              ZZZZzzzzzzzz (12 bits) : effect and argument
	...
	for (i=0;i<mod_channels_num;i++) {    		
			...
			if (period||instr) {
    			...
    			if (period) {
    				...
    				// switch off running effects --------
    			mod_channels[i]->volume_slide=0;
   			}
    		...
    		switch (effect) {     
				...
				case 0x0:	// arpeggio effect ---------------------- xxxx yyyy
					...
					break;	
				case 0x6:  // volume slide while vibrato....	
				case 0xA:  // volume slide effect ------------------------ xxxx yyyy
					
						mod_channels[i]->volume_slide=-(effect_arg&0xf)+(effect_arg>>8);	
					
					break;		  					
					
				case 0x1:
						mod_channels[i]->portamento = -effect_arg;  //decrease period every line-tick
						mod_channels[i]->volume_slide=0;
						break;
				case 0x2:
						mod_channels[i]->portamento = effect_arg;   //increase period every line-tick
						mod_channels[i]->volume_slide=0;
						break;
				default:
					mod_channels[i]->volume_slide=0;
    ...
    for (i=0;i<mod_channels_num;i++) {
		...
		if (mod_channels[i]->volume_slide) mod_channels[i]->volume+=mod_channels[i]->volume_slide;
		
		if (mod_channels[i]->volume>=MAX_VOLUME) mod_channels[i]->volume=MAX_VOLUME-1;
		if (mod_channels[i]->volume<0) mod_channels[i]->volume=0;
...
void interrupt_mixer(void) {
	...
	//s16 channelL4,channelR4;
	
	s16 CL8,CR8;
	
//if sound stopped - do exit to
	if ( already_mixing || (st_snd==0)) return;   // a lock for not to enter twice the mixin routine
	already_mixing=1;
...
//Profile("modmix",
	...
	//clear the L&R buffers before mixing channels on them
		...
		for (i=0;i<BUFFER1_SIZE/2;i+=2) {
			//channelL4=writeTo[i  ]>>2;
			//channelR4=writeTo[i+1]>>2;
			
			//writeTo[i  ]=0x8000+(channelL4<<1)+channelL4+channelR4;  // to unsigned values....
			//writeTo[i+1]=0x8000+(channelR4<<1)+channelR4+channelL4;  // to unsigned values....

			CL8=writeTo[i  ]>>3;
			CR8=writeTo[i+1]>>3;
#ifdef FILTER_ON			
			FLAST_L=(FLAST_L>>1)+(((CL8<<2) + (CL8<<1) + (CR8<<1) + CR8)>>1);
			FLAST_R=(FLAST_R>>1)+(((CR8<<2) + (CR8<<1) + (CL8<<1) + CL8)>>1);
			writeTo[i  ] = 0x8000 + FLAST_L ;
			writeTo[i+1] = 0x8000 + FLAST_R ;
#else
			writeTo[i  ] = 0x8000 + (CL8<<2) + (CL8<<1) + (CR8<<1) + CR8;
			writeTo[i+1] = 0x8000 + (CR8<<2) + (CR8<<1) + (CL8<<1) + CL8 ;
#endif
    ...
    already_mixing=0;  //unlock the IRQ entrance
...
///////////////////////////////////////////////////////////////////////////
// sound_init(use_timer, timer_num)
//
// use_timer = {true,false}    = hook a timer to interrupt_mixer
// timer_num				   = which timer to hook on..
//

void sound_init(int use_timer_param,int timer_num_param) {
	...
	use_timer = use_timer_param;   //keep the config for later use in:
	timer_num = timer_num_param;   //     start_sound,  stop_sound  procedures
	
	buffer_size=BUFFER1_SIZE*NUMBUFFERS;
	
	buffer=(u16 *)gp_mem_func.malloc(buffer_size);
	
	bsize=buffer_size>>1;
	for (i=0;i<bsize;i++) buffer[i]=0x8000; 
	...
	for (volume=0;volume<MAX_VOLUME;volume++) { //build volume table
    		for (i=-128;i<128;i++)
		    	...
		    	volume_table[volume][((u8)i)]=(((i/*+0x80*/)*volume)<<(8-6))/(max_channels+max_sfx_channels-2) ; // unsigned= +0x80
	}
...
int sfx_play(s8 *sfx_data, int length, float rate, s8 paning, float pan_increment_50th, u32 volume) {
	...
	if (channel_allocated!=-1) {
		...
		sfx_channels[channel_allocated].length = length<<16;
...
/////////////////////////////////////////////////////////////////////////////
//int is_sfx_playing(void)
//
// returns TRUE if there's a playing sfx channel(s)
//
// returns: true / false ... if it was stopped or not
int is_sfx_playing(void) {
	int i;
//geepee32 hangs here.. and it's cool.. cuz it works on the hw
#ifdef NOEMU	
	for (i=0; i<=max_sfx_channels; i++) {
		if (sfx_channels[i].playing)
			return true;
	}
#endif
...
*/
		...
		mod_instrument[i].volume_slide=0;
