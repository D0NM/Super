...
output_channel *mod_channels[]={	
				&channels_L[0],&channels_R[0],
...
void mod_do_pattern_break (int new_pattern_pos, int pattern_n) {
	int i;
...
//              ZZZZzzzzzzzz (12 bits) : effect and argument
    	...
    	if (effect_jump) 
            ...
            if (mod_pattern_row>63) mod_do_pattern_break(0,mod_pattern_n+1); // to line 0
        }
...
void mix_channel(output_channel *channel, s16 *buffer_out) {
...
void interrupt_mixer(void) {
	...
	//*(volatile unsigned long*)0x14A007fc^=0xffffffff;
	//	__printf("interrupt_mixer, writting_buffer=0x%x\n",writting_buffer);
		...
		for(channel_n=0;channel_n<max_channels;channel_n++) {    //MIX the L & R channels
    		...
    		if (channels_R[channel_n].playing)
    			mix_channel(&(channels_R[channel_n]), (s16 *)(writeTo));
...
void sound_init(void) {
	...
	mod_playing=false;
	writting_buffer=0;
	gp32sound_init((short int *)buffer,buffer_size,BUFFER1_SIZE);
    ...
    for (volume=0;volume<MAX_VOLUME;volume++) { //build volume table
    	for (i=-128;i<128;i++)
    	volume_table[volume][((u8)i)]=(((i/*+0x80*/)*volume)<<(8-6))/(max_channels*2) ; // unsigned= +0x80
...
/////////////////////////////////////////////////////////////////////////////
//
// big_e_32	     big endian to little endian u16/u32 converters
// big_e_16
//  
...
/*	GpPcmPlay((unsigned short *)buffer,buffer_size,1);
	...
	if (use_timer) {
         	GpTimerOptSet(timer_num, 55, 0, interrupt_mixer);
